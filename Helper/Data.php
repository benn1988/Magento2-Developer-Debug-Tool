...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function setObserverDetails($configuration, $eventName)
    {
        ...
        if ($data=$this->getDeveloperRegistry()) {
            ...
            $data[$this->eventDetailsKey][$observerKey]=['configuration'=>$configuration,'event'=>$eventName,'count'=>$count];
        } else {
            $data=[];
            $data[$this->eventDetailsKey][$observerKey]=['configuration'=>$configuration,'event'=>$eventName,'count'=>$count];
        }
    ...
    public function addBlockInfo($handle, $class, $template)
    {
        ...
        if ($data=$this->getDeveloperRegistry()) {
            $data['block_details'][]=['handle'=>$handle,'class'=>$class,'template'=>$template];
        } else {
            $data=[];
            $data['block_details'][]=['handle'=>$handle,'class'=>$class,'template'=>$template];
        }
